      ...
      <table class="data-table">
        ...
        <tbody>
          <tr v-for="e in list" :key="e.id">
            <td>{{ e.id }}</td>
            <td>{{ e.name }}</td>
            <td><router-link :to="`/entry/${e.id}`" class="btn">{{ commonLang.view }}</router-link></td>
          </tr>
        </tbody>
      </table>
...
import { common as commonLang, entry as entryLang } from '@/lang/zh-cn'
